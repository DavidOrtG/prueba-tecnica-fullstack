...
const options = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description:
        'API para sistema de gestión financiera con autenticación y control de acceso basado en roles',
      contact: {
        name: 'API Support',
      ...
      schemas: {
        ...
        Error: {
          ...
          properties: {
            ...
            details: {
              type: 'object',
              example: { field: 'Missing required field' },
            },
          },
          required: ['error'],
        },
  ...
  try {
    ...
    if (session.user.role !== 'ADMIN') {
      return res.status(403).json({
        error: 'Forbidden: Admin access required to view API documentation',
      });
    }
